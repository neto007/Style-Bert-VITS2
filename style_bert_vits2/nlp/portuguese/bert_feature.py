...
def extract_bert_feature(
    ...
    """
    Portuguese text to BERT feature (PyTorch)
    """
    ...
    if device == "cuda" and not torch.cuda.is_available():
        device = "cpu"
    model = bert_models.load_model(Languages.PT, device_map=device)
    bert_models.transfer_model(Languages.PT, device)
...
def extract_bert_feature_onnx(
    ...
    """
    Portuguese text to BERT feature (ONNX)
    """

    # Load tokenizer and model
    tokenizer = onnx_bert_models.load_tokenizer(Languages.PT)
    ...
    if assist_text:
        ...
        io_binding = session.io_binding()  # Recreate IOBinding
